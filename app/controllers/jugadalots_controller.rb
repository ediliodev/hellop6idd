       ...
       case entrada
        ...
        when /^\d{4}$/  # Si son cuatro digitos Old quiniela command: when /^\d{2}\+\d+$/   ej. 02+100 #Verificar si es Quiniela
         ...
         when /^\/ct\d+\+\d+$/   # Consultar ticket "/at123456+8855522": ganador (pendiente de pago), ganador (pago), nulo. perdedor, en proceso de resultados para procesar.
              ...
              #verificar que no sea nil el query para verificar que el ticket sea impreso o no ok./ct status tambien ok.
              if not @t_valido.nil? # si existe y no se ha impreso
                  if @t_valido.fechaimpresion == nil
                    redirect_to "/jugadalots/new", notice: "X Ticket No valido. NotprintedRandom check" and return                   
                  end              
              end

              if @t_valido.nil?
                redirect_to "/jugadalots/new", notice: "X Ticket No valido. Ok.." and return 
              end
