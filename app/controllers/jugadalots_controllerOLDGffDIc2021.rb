class JugadalotsController < ApplicationController
  ...
  def new
    ...
    if  session[:ticket_id] != nil # Me aseguro que el ticket exista antes de asignarlo. ok. 
      ...
      redirect_to "/jugadalots/new", notice: "X. No tk_id, favor borrar todo (/bt) pruebe de nuevo" and return 
    end
    # -------------------------------------------------------------------
...
=end
   ...
   entrada.each do |entrada|
        ...
        when /^\d{4}$/  # Si son cuatro digitos Old quiniela command: when /^\d{2}\+\d+$/   ej. 02+100 #Verificar si es Quiniela
         ...
         when /^\.{2}\d+$/ # Ej. "..500" procesar regsitro e impresion del ticket. Este si va a la venta.
          ...
          #redirect_to "/jugadalots/new", notice: "probando 123..." and return # todos.
            ...
            @pelea_cerrada = verificar_pelea_cerrada(last_tickect_current_user()) 
            # if @sorteo_cerrado
            if (@pelea_cerrada.size > 0) # el objeto es el tipo array.size method is ok
            ...
            if excede_control_de_pago_x_ticket(last_tickect_current_user, entrada) # posible_pago = procesar_posible_pago(@jugadalot.ticket_id, entrada) # la funcion retorna nil, o array con 3 elementos ok. Lo mando porque voy a invocarla dentro de esta funcion que acabo de llamar ok.
              redirect_to "/jugadalots/new", notice: "X. Monto excede maximo x ticket." and return # 
            end

            #verificar limites de pago global
            if excede_control_de_pago_global(last_tickect_current_user, entrada)
              redirect_to "/jugadalots/new", notice: "X. Monto excede Maximo global." and return # 
            end
         ...
         when /^\.\d{2}+$/ # Setear loteria manual
          ...
          when /^\.{2}\d+$/ # Verificar si manda a imprimir ticket y monto recibido.
            ...
            if session[:total_tk_viejo].to_i >  @efectivo_recibido # falta dinero
              ...
              redirect_to "/jugadalots/new", notice: " Cantidad Recibida " + "($#{@efectivo_recibido})" + " no es suficiente, es menor al total del ticket. Falta efectivo." and return  # antes era asi ,es menor al total del ticket. Faltan $" + "#{@diferencia_efectivo} pesos" and return  Pero en modo  de venta mixta automatica la sumatoria no da por la session total, por lo que por ahora ponemos que falta dinero (pesos) no importa la cantidad. En la pantalla le sale el total del ticket y el monto recibido. Ok. ted.
            end
      ...
      #Verificar si es Quiniela
        ...
        when /^\d{2}\+\d+$/ # ej. 02+100 #Verificar si es Quiniela
            @comado_jugada = entrada + "." + session[:loteria_manual].to_s # comando completo ej. 02+100 y agrego la sigla  del sorteo ..01,.02?? ok.
        ...
        when /^\d{2}\+\d+\.\d{2}$/  # ej. 02+100.02 Verificar quiniela con sorteo manual especifico.
            ...
            if @verificar_sorteo_en_quiniela
              ...
              redirect_to "/jugadalots/new", notice: "No se pudo grabar esta jugada porque el sorteo: [#{@cadena[1].split(".")[1]}] no existe. Favor trate de nuevo." and return 
            end
      ...
      #Verificar si es Pale
        when /^\d{4}\+\d+$/ # ej. 0220+100 #Verificar si es Pale
            @comado_jugada = entrada + "." + session[:loteria_manual].to_s 
            ...
            @jugadalot.n2 = @n2
            @jugadalot.monto = @monto.to_i
            @jugadalot.comandojugada = @comado_jugada
            ...
            if session[:loteria_manual] == nil
             ...
             #VERIFICAR SI ES SUPER PALE, ENTOCES LE PONEMOS SP EN EL REGISTRO E IMPRESION DEL TK?
             #if @jugadalot.sorteot_id.to_i == 13 || @jugadalot.sorteot_id.to_i == 14 || @jugadalot.sorteot_id.to_i == 17 # Si es un sorteo super pale, no un pale normal de cualquier 'loteria' ok. ted. le ponemos que es un super pale, mu diferente a un pale, empezando por el pago, etc..
             #   @jugadalot.qpt = "SP"
             #end

             #verificar disponibilidad de Pale.
              if verificar_disponibilidad_simple(@jugadalot) < 0
                redirect_to "/jugadalots/new", notice: "Cantidad no disponible para ese Pale: #{entrada}"  and return 
              end
      ...
      #Verificar si es Tripleta
        when /^\d{6}\+\d+$/ # ej. 010203+100 Verificar si es Tripleta
            ...
            if verificar_disponibilidad_simple(@jugadalot) < 0
              redirect_to "/jugadalots/new", notice: "Cantidad no disponible para esa Tripleta: #{entrada}"  and return 
            end
        ...
        when /^\d{6}\+\d+\.\d{2}$/ # ej. 010203+100.02 Verificar si es Tripleta en sorteo especifico.
            ...
            if @verificar_sorteo_en_tripleta
              ...
              redirect_to "/jugadalots/new", notice: "No se pudo grabar esta jugada porque el sorteo: [#{@cadena[1].split(".")[1].to_s}] no existe. Favor trate de nuevo." and return 
            end
          ...
          when "/g"  # Consultar numeros ganadores de Hoy.
            ...
            #@numeros_ganadores = Ganadorest.where(:fecha => Date.today) # Poner la -4 horas klk. o usar la gema especial de las fechas de reportes. GMT-4 ok ted pdte feb 2019. Asi by_day(@dia) ok resuelto? probarlo.
            
            @numeros_ganadores = Ganadorest.where(:fecha => Time.now.to_date.to_time) # esto porque Date.today retorna la fecha de hoy pero al inicio de las 00:00:00 de la noche. ok ted. Probar en rails console Date.now vs Time.now.to_date, Time.now + 2.hours, etc..

            if @numeros_ganadores.empty?
              redirect_to "/jugadalots/new", notice: "No se han procesado Ganadores. Por favor trate mas tarde o contacte a la Central." and return
            end
         ...
         when /^\/at\d+\+\d+$/ # Anular ticket "/at123456+8855522" 
              ...
              if (@t_valido.user_id != current_user.id) && (current_user.tipousuariot_id != 2) # || (not current_user.admin? o !=2 ok.)
                redirect_to "/jugadalots/new", notice: "X Este usuario no puede anular este ticket. No eres admin ni el ticket es de esta sucursal. Favor contacte Central para su anulacion " and return 
              end
         ...
         when /^\/pt\d+\+\d+$/  # /pt pago de ticket. ej.  /pt321+4068295371 (pagar ticket + serail)
              ...
              if (@t_valido.activo == "si" && @t_valido.ganador == "si" && ( @t_valido.pago == "no" || @t_valido.pago == nil) )
                  ...
                  #Revisar que el ticket no tenga mas jugadas con sorteos abiertos pendientes por resultados antes de proceder a su pago:
                  #Jugadalots 
                  #Tickets
                  #Sorteots
                  @revisar_jugadas_sorteos = Jugadalot.where(:ticket_id => @t_valido.id) 
                  #Sorteo cerrado o abierto pero abierto con fecha de dia siguiente al del ticket?
                  
                  @sorteos_abiertos = []

                  @revisar_jugadas_sorteos.each do |jugada|
                    if (jugada.sorteot.abierto == "si") && (jugada.sorteot.updated_at < jugada.updated_at )
                      #hay sorteos abiertos, recolectarlos y no permitir pago hasta que se cierren dichos sorteos.Pediente considerar que se computen ganadores. ( Se entiende que deben digitar los ganadores al momento del cierre del sorteo, por eso saldria el monto al momento de consultar un tk ganador en varios sorteos procesados. ok Se menciona lo de considerar los ganadores como una posibilidad del programa, pero no es necesario por la logia de funcionamiento de la aplicacion y la persona encargada de cierre y computacion de ganadores del sorteo. ok ted.)

                      @sorteos_abiertos << jugada.sorteot.nombre.to_s
                    end  
              
                  end
                  
                  if not @sorteos_abiertos.empty?
                      @sorteos_abiertos = @sorteos_abiertos.join(",")
                      redirect_to "/jugadalots/new", notice: "X - Este ticket aun tiene sorteos abiertos: [ #{@sorteos_abiertos} ] y no puede ser pagado en su totalidad. Debe esperar los resultados de los mismos o, en otra ocasion, realizar tickets por separados (para cada sorteo) si desea el cobro individual. " and return                       
                  end
  ...
  def excede_control_de_pago_global(last_tickect_current_user, entrada)
    #verificar tipo de parlay de ese ticket:
    parlay_count = Jugadalot.where(:ticket_id => last_tickect_current_user).count
    #parlay_count
    ...
    #limite GLOBAL x parlay OK:
    control_monto_max_pago = Controldepagogt.where(:tipojugada => tipo_jugada ).first.limiteglobal.to_i


    #MODIFICAR ESTA SECCION PORQUE EL RIESGO ES EL GLOBAL DE HOY.
    #-----------------------------------------------------------------
    #verificar posible pago: riesgo
    #riesgo global es la suma de todos los posibles pagos de cada ticket de determinado parlay.
    hash_tikets_de_hoy = Jugadalot.today.group_by(&:ticket_id) # retorna un hash con los tickets de hoy agrupados por ticket id. Ahora hay que sumar posible pago de caga ticket para determinar el riesgo global de hoy ok.

    #determinar suma del riesgo global x parlay
    suma_riesgo_global = 0
    #suma_riesgo_global_directo = 0
    #suma_riesgo_global_pale = 0
    #suma_riesgo_global_tripleta = 0
    #suma_riesgo_global_cuarteta = 0
    #suma_riesgo_global_quinteta = 0
    #suma_riesgo_global_sexteta = 0
    #suma_riesgo_global_septimeta = 0
    #suma_riesgo_global_octaveta = 0

    
    hash_tikets_de_hoy.each do |ticket_key, todas_jugadas_value|
     
      tipo_parlay = todas_jugadas_value.size #hash_tikets_de_hoy[ticket_key].size # determino tipo_parlay de ese ticket
      if tipo_parlay.to_i == parlay_count.to_i # Sigifica que sumaremos los globales del mismo tipo de parlay del current ticket user, o sea determinal suma global de pales por ejemplo si el current ticket es un pale ok. ted.
        #somar posible riesgo global de cada directo encontrado de hoy:
        suma_riesgo_global += todas_jugadas_value[0].posiblepago.to_i #hash_tikets_de_hoy[ticket_key][0].posiblepago
      end
    
    end # end del each loop ok

    #
    entrada = entrada[1..(entrada.size-1)] # "SOBREESCRIBO ESTO ASI: ..250 => .250" ESTO PARA QUE LUEGO SE TRANSFORME DE .250 A 250 DENTRO DE LA FUNCION. REUCUERDA QUE EN ESTA SECCION DE CODIGO ENTRADA DE DE COBRO FINAL NO DE EVALUACION DE PAGO: ..250 ES DE COBRO FINAL. OK. TED.
    riesgo = procesar_posible_pago(last_tickect_current_user, entrada)# procesar_posible_pago retorna array ok ted.
    #-----------------------------------------------------------------
    #riesgo += suma_riesgo_global # reasignacion de valores para cambio de variables y seguir la misa logica desarrollada ok.

    if (riesgo != nil) 
      if ( (riesgo[1].to_i + suma_riesgo_global) <= control_monto_max_pago)
        false # no excede ok
      else
        true # execede
      end
    end # end del condition if (riesgo != nil) 
  ...
  def verificar_pelea_cerrada(ticket)
    @jugadalots = Jugadalot.where(:ticket_id => ticket.id) || nil # 
    peleas_cerradas = []

    @jugadalots.each do |jugada|
      status_pelea = Lineat.today.where(:pelea => jugada.pelea.to_i).first.status || nil
      if status_pelea != "abierta"
        peleas_cerradas << jugada.pelea.to_i
      end
    
    end

    peleas_cerradas # retorno el array de pelas cerrdas. Si el size es 0, entonces no hay cerradas y por ende estan todas abiertas, o sea no empezadas y pueden recibir apuestas. procesar en la logica del codigo que invoca esta funcion ok.
